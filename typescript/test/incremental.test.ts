...
test("first run adds all docs; second run is a no-op (idempotent)", () => {
  const dir = vault();
  try {
    writeFileSync(join(dir, "a.md"), "# Alpha\nbody\n");
    writeFileSync(join(dir, "b.md"), "# Beta\nbody\n");

    const first = updateMemory(dir);
...
test("only the changed doc is updated; others stay unchanged", () => {
  const dir = vault();
  try {
    writeFileSync(join(dir, "a.md"), "# Alpha\nbody\n");
    writeFileSync(join(dir, "b.md"), "# Beta\nbody\n");
...
test("a removed doc is reported and dropped from the graph", () => {
  const dir = vault();
  try {
    writeFileSync(join(dir, "a.md"), "# Alpha\nbody\n");
    writeFileSync(join(dir, "b.md"), "# Beta\nbody\n");
